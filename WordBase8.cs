...
{
    class WordBase8 : IDisposable
    {
        public string LastError { get; set; }
        public Application wordApp8 { get; set; }
        ...
        public void Export(String pTemplateFileName, String pTargetFileName, System.Data.DataTable pDt, String pFileType, String pIsTempFile)
        {
            ...
            if (pDt != null && pDt.Rows.Count > 0)
            {
                ...
                {
                    string folderPath = System.Windows.Forms.Application.StartupPath + @"\" + "SMA0-4";
                    Console.WriteLine(string.Format("{0} 從範本複製一份到新檔案(SMA 0-4)", DateTime.Now));
                    //從範本複製一份到暫存檔案
        ...
        public void CreateFile(String pTemplateFileName, string pTempFileName, string folderPath, bool IsVisible = false)
        {
            ...
            //是否顯示處理過程
            wordApp8 = new Application { Visible = IsVisible };

            //開啟檔案
            string OutputFile = string.Format("{0}\\{1}", folderPath, pTempFileName);
            aDoc = wordApp8.Documents.Open(OutputFile, ReadOnly: false, Visible: IsVisible);
        ...
        public void GotoTheBegining()
        {
            try
            {
                wordApp8.Selection.HomeKey(unit, missing);
            }
            catch (Exception)
            {
            }
        ...
        public void Replace(Object findText, Object replaceText)
        {
            try
            {
                ...
                if (replacetxt_ary.Length == 1 || (replacetxt_ary.Length > 1 && replacetxt_ary[1] == "Color"))
                {
                    ...
                    //替換全域性Document
                    wordApp8.Selection.Find.ClearFormatting();
                    wordApp8.Selection.Find.Replacement.ClearFormatting();
                    wordApp8.Selection.Find.Text = findText.ToString();
                    replaceText = replacetxt_ary[0];
                    wordApp8.Selection.Find.Replacement.Text = replaceText.ToString();
                    ...
                    if (replacetxt_ary.Length > 2)
                    {
                        if (replacetxt_ary[1] == "Color")
                        {
                            ...
                            {
                                case "RED":
                                    wordApp8.Selection.Find.Replacement.Font.ColorIndex = WdColorIndex.wdRed;
                                    break;

                                case "BLUE":
                                    wordApp8.Selection.Find.Replacement.Font.ColorIndex = WdColorIndex.wdBlue;
                                    break;

                                case "LIGHTBLUE":
                                    wordApp8.Selection.Find.Replacement.Font.Color = (WdColor)((192 << 16) + (112 << 8) + 0); // 使用 RGB 值 (R=0, G=112, B=192)
                                    break;
                            }
                        }
                    }

                    wordApp8.Selection.Find.Execute(ref findText, ref matchCase, ref matchWholeWord,
        ...
        private void FindReplaceHeaderTxt(object findText, object replaceText)
        {
            try
            {
                ...
                if (replacetxt_ary.Length > 1 && replacetxt_ary[1] == "Header")
                {
                    ...
                    wordApp8.ActiveDocument.Sections[1].Headers[WdHeaderFooterIndex.wdHeaderFooterPrimary].Range.Find.Execute(
                       ref findText,
        ...
        public void SearchReplacePicInHeaderAndTextBox(string FindStr, object replacePic, object W, object H)
        {
            ...
            //{
                // 取得當前文檔的所有節
                foreach (Microsoft.Office.Interop.Word.Section section in this.wordApp8.ActiveDocument.Sections)
                {
                    // 取得節的主要頁首範圍
                ...
                // 確保在頁首或頁尾模式時返回到主文件
                if (wordApp8.ActiveWindow.View.Type == WdViewType.wdPrintView)
                {
                    wordApp8.ActiveWindow.ActivePane.View.SeekView = WdSeekView.wdSeekMainDocument;
                }
            //}
            //catch (Exception ex)
            //{
        ...
        private void FindReplaceHeaderTxtAndTextBox(object findText, object replaceText)
        {
            try
            {
                ...
                foreach (Shape shape in wordApp8.ActiveDocument.Sections[1].Headers[WdHeaderFooterIndex.wdHeaderFooterPrimary].Range.ShapeRange)
                {
                    // 判斷是否為文字方塊
                    if (shape.Type == Microsoft.Office.Core.MsoShapeType.msoTextBox)
                    {
        ...
        public void SearchReplacePic(string FindStr, object replacePic, object W, object H)
        {
            ...
            {
                GotoTheBegining();
                this.wordApp8.Selection.Find.ClearFormatting();
                if ((this.wordApp8.Selection.Find.Execute(FindStr) == true))
                {
                    this.wordApp8.Selection.Select();
        ...
        public void Close()
        {
            ...
            if (wordApp8 != null)
            {
                ...
                ((_Application)wordApp8).Quit(ref dontSave);
                //}
                //finally
                //{
                Marshal.FinalReleaseComObject(wordApp8);
                //}
            }
            aDoc = null;
            wordApp8 = null;
